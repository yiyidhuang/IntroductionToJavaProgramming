...
public class ModifyTree extends JApplet {
	...
	public ModifyTree() {
		...
		p2.add(new JScrollPane(jTree2 = new JTree(new DefaultTreeModel(root))));  // Same root as jTree1
		...
		jbtRemove.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < paths.length; i ++) {
					DefaultMutableTreeNode node = (DefaultMutableTreeNode)(paths[1].getLastPathComponent());
					
					if (node.isRoot()) {
						JOptionPane.showMessageDialog(null, "Cannot remvoe the root");
					} else {
