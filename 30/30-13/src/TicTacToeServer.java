...
public class TicTacToeServer extends JFrame 
	...
	public TicTacToeServer() {
		...
		try {
			...
			while (true) {
				jtaLog.append(new Date() + ": Wait for players to join session " +
							sessionNo + '\n');
				...
				// Notify that the player i Player 2
				new DataOutputStream(
						player2.getOutputStream()).writeInt(PLAYER2);
				
				// Display this session and icrement session number
...
class HandleASession implements Runnable, TicTacToeConstants {
	...
	/** Implement the run() methods for the thread */
	public void run() {
		try {
	...
	private boolean isFull() {
		for (int i = 0; i < 3; i ++) {
			for (int j = 0; j < 3; j ++) {
				if (cell[i][j] == ' ') {
					return false;  // At least one cell is no filled
				}
			}
		}
